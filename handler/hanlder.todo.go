...
func FetchSingleTodo(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"status": http.StatusOK, "data": model.TransformedTodo{ID: todo.ID, Title: todo.Title, Completed: completed}})
	return
}
...
func UpdateTodo(c *gin.Context) {
	...
	db.DB.Model(&todo).Update("title", c.PostForm("title"))
	completed, _ := strconv.Atoi(c.PostForm("completed"))
	db.DB.Model(&todo).Update("completed", completed)
	c.JSON(http.StatusOK, gin.H{"status": http.StatusOK, "message": "Todo updated succesfully"})
}
